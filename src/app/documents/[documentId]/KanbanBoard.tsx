...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ isOpen, onClose, boardId }) => {
  const boardRef = useRef<HTMLDivElement>(null);
  const boardData = useQuery(api.kanban.getBoard, { boardId });
  ...
  const handleDrop = async (columnId: Id<"kanbanColumns">) => {
    ...
    await moveTask({
      ...
      newPosition: 0, // You might want to calculate based on drop position
    });
